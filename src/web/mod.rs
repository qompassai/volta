...
pub struct VoltaTemplate(&'static str, serde_json::Value, I18n, RequestOrigin);

impl<'r> Responder<'r, 'static> for VoltaTemplate {
    fn respond_to(
        self,
        req: &'r rocket::Request,
    ) -> std::result::Result<Response<'static>, Status> {
        let VoltaTemplate(tmpl, ctx, i18n, origin) = self;
        ...
        let layout_context = templates::VoltaLayout::new(ctx, i18n, origin);
...
impl MyResponse {
    pub fn ok(tmpl: &'static str, ctx: impl Serialize, i18n: I18n, origin: RequestOrigin) -> Self {
        ...
        MyResponse::Success(VoltaTemplate(tmpl, context_json, i18n, origin))
    }
    ...
    pub fn xml(tmpl: &'static str, i18n: I18n, origin: RequestOrigin) -> Self {
        let context_json = serde_json::to_value(templates::Bare { dummy: () }).unwrap();
        MyResponse::Xml(VoltaTemplate(tmpl, context_json, i18n, origin))
    }
    ...
    pub fn bad_request(
        ...
        let context_json = serde_json::to_value(ctx).unwrap();
        MyResponse::BadRequest(VoltaTemplate(template, context_json, i18n, origin))
    }
    ...
    pub fn not_found(
        ...
        let context_json = serde_json::to_value(ctx).unwrap();
        MyResponse::NotFound(VoltaTemplate(
...
mod templates {
    ...
    #[derive(Serialize)]
    pub struct VoltaLayout<T: serde::Serialize> {
    ...
    impl<T: serde::Serialize> VoltaLayout<T> {
...
pub struct VoltaState {
    /// Assets directory, mounted to /assets, served by volta or nginx
    assets_dir: PathBuf,
...
impl<'r> request::FromRequest<'r> for RequestOrigin {
    ...
    async fn from_request(
        ...
        let volta_state = request.rocket().state::<VoltaState>().unwrap();
        let result = match request.headers().get("x-is-onion").next() {
            Some(_) => RequestOrigin::OnionService(volta_state.base_uri_onion.clone()),
            None => RequestOrigin::Direct(volta_state.base_uri.clone()),
        };
...
#[get("/assets/<file..>")]
async fn files(file: PathBuf, state: &rocket::State<VoltaState>) -> Option<NamedFile> {
...
fn errors(
    ...
    let response_body = Template::render(
        ...
        templates::VoltaLayout::new(templates::Bare { dummy: () }, i18n, origin),
    );
...
fn rocket_factory(
    ...
    let volta_state = configure_volta_state(figment)?;
...
fn configure_volta_state(config: &Figment) -> Result<VoltaState> {
