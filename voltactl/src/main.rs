...
fn main() -> Result<()> {
    let matches = App::new("Volta Control")
        .version("0.1")
        .about("Control volta database externally")
        .arg(
    ...
    let configs: VoltaConfigs = toml::from_str(&config_data).unwrap();
